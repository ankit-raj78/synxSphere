...
const RoomRecommendations = forwardRef<RoomRecommendationsRef, RoomRecommendationsProps>(
  ({ userId, refreshTrigger }, ref) => {
        ...
        if (response.ok) {
          ...
          const roomRecommendations: RoomRecommendation[] = rooms.map((room: any) => ({
            room: {
              ...
              isOwnRoom: room.creatorId === currentUserId,
              musicalRequirements: {
                instruments: [],
                genres: [room.genre],
    ...
    const joinRoom = async (roomId: string) => {
      ...
      try {
        const token = localStorage.getItem('token')
          // Check if it's user's own room
        const room = recommendations.find(rec => rec.room._id === roomId)?.room
        if (room?.isOwnRoom) {
          // Enter own room directly
                  ...
                  >
                    {joining === rec.room._id ? (
                      <>
                        ...
                        <span>{rec.room.isOwnRoom ? 'Entering...' : 'Joining...'}</span>
                      </>
                    ) : (
                      <>
                        <span>{rec.room.isOwnRoom ? 'Enter Room' : 'Join Collaboration'}</span>
